...
| Canary artifact | FW-120 compiler fixture suite; see section 6.4 | every pull request through `pnpm run test` | exact-byte production build tests |
| Repository secret | hosting-provider scanner with push protection | repository-owner configuration | provider evidence; unavailable locally |
...
`SEC-SUPPLY-002` requires secret scanning and `docs/09_SECURITY_PRD.md` section 26 requires no secret-scanning finding on every pull request. FW-120 provides a bounded exact-byte scanner and production-build fixtures for explicitly supplied canaries. Those tests run through the ordinary runtime suite, but they are not general credential discovery and do not scan repository history.

The repository owner must still enable the hosting provider's secret scanning with push protection, which is a configuration action rather than framework code. Client/server taint analysis and production log/error redaction remain FW-212 and FW-217. Until provider execution and those downstream controls exist, the complete requirement remains covered by `AR-001`. Per `docs/SECURITY_EVIDENCE_WORKFLOW.md` section 3.1, configured or locally passing checks become release evidence only after the blocking CI gate executes.
