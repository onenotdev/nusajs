...
Since FW-019, `.github/workflows/ci.yml` also defines a dependency-vulnerability gate and a license gate, and `docs/SUPPLY_CHAIN_POLICY.md` sections 5, 6.2, and 10 define what they block on. Defined is not the same as enforceable: continuous integration has never executed in this working copy, and section 3.1 of this document holds that an automated check counts as evidence only once it runs on every pull request and its failure blocks merge. So the two dependency gates are specified and locally exercised through `pnpm run verify`, while the secret-scanning gate does not exist yet and the remaining rows still depend on discipline. That is the expected M0 state, recorded in the FW-008 approval, section 5, and deferred under `AR-001`.
