...
- ADR-007: Error-code taxonomy. `Proposed` — `docs/adr/ADR-007-error-code-taxonomy.md`. Proposes `NUSA-<SUBSYSTEM>-<NUMBER>` with a registered subsystem vocabulary, per-subsystem monotonic allocation, permanent tombstones, deterministic documentation paths, and no security-diagnostic suppression; implementation is blocked pending core-maintainer acceptance.
...
- ADR-018: Core package and public entry points. `Proposed` — `docs/adr/ADR-018-core-package-and-public-entry-points.md`. Proposes a private ESM-only `@nusajs/core` package with explicit stable and experimental entry points, no wildcard or public `internal/*` exports, no initial facade, and no Node.js or workspace runtime dependencies; FW-101 implementation is blocked pending core-maintainer acceptance.
