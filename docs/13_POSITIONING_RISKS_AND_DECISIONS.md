...
- ADR-007: Error-code taxonomy. `Accepted` — `docs/adr/ADR-007-error-code-taxonomy.md`. Selects `NUSA-<SUBSYSTEM>-<NUMBER>` with a registered subsystem vocabulary, per-subsystem monotonic allocation, permanent tombstones including experimental codes, deterministic documentation paths, and no security-diagnostic suppression; accepted by the core maintainer on 2026-08-17.
...
- ADR-018: Core package and public entry points. `Accepted` — `docs/adr/ADR-018-core-package-and-public-entry-points.md`. Selects a private ESM-only `@nusajs/core` package with explicit exports, no wildcard or public `internal/*` exports, no initial facade, and no Node.js or workspace runtime dependencies; the optional experimental entry point remains absent until useful; accepted by the core maintainer on 2026-08-17.
